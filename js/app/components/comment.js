...
export const comment = (() => {
    ...
    const onNullComment = () => {
        const desc = lang
            ...
            .on('pt', '📢 Compartilhe este convite para receber mais mensagens! 🎉')
            .get();
    ...
    const showMore = (anchor, uuid) => {
        ...
        anchor.innerText = isCollapsed ? 'Ver menos' : 'Ver mais';
    ...
    const cancel = async (button, id) => {
        ...
        if (gif.isOpen(id) && ((!gif.getResultId(id) && isChecklist === isPresent) || util.ask('Tem a certeza?'))) {
            await gif.remove(id);
            removeInnerForm(id);
            return;
        }
        ...
        const form = document.getElementById(`form-inner-${id}`);
        if (form.value.length === 0 || (util.base64Encode(form.value) === form.getAttribute('data-original') && isChecklist === isPresent) || util.ask('Tem a certeza?')) {
            removeInnerForm(id);
            return;
        }
